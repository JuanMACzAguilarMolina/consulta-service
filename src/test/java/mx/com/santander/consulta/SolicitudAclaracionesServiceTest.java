...
class SolicitudAclaracionesServiceTest {
	...
	@Mock
	SolicitudAclaracionesRepository repository;

	@InjectMocks
	SolicitudAclaracionesService service;
	...
	void solicitudAclaracionesGet() throws Exception {
		...
		request.setNumPol("14573");

		Mockito.when(repository.getConsulta(request)).thenReturn(consulta);
		service.getConsulta(request);
	}
