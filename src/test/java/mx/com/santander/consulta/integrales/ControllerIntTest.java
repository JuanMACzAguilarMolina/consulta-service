...
class ControllerIntTest {
	...
	private final static String REQUEST_ACLARACIONES2 = "{\n" + "                \"idApp\": \"IR\",\n"
			+ "                \"idCanal\": \"4\",\n" + "                \"idRamo\": \"15\",\n"
			+ "                \"numPol\": \"14574\"\n" + "}";

	private ConsultaSolicitudAclaracionesResponse infoAclaracion = null;
	...
	void setUp() throws ParseException {
		...
		infoAclaracion = new ConsultaSolicitudAclaracionesResponse();
	...
	void consultaAclaraciones() {
		...
		when(jdbcTemplate.queryForObject(eq(consultaReporte.getGetAclaraciones()),
				eq(ConsultaSolicitudAclaracionesResponse.class), ArgumentMatchers.any())).thenReturn(infoAclaracion);

		callService(HttpMethod.POST, 200, Constants.ENDPOINT_GENERAL_ACLARACIONES + Constants.ENDPOINT_CONSULTA,
				REQUEST_ACLARACIONES);
	}

//	@Test
//	void consultaAclaracionesError() {
//		when(jdbcTemplate.queryForObject(eq(consultaReporte.getGetAclaraciones()),
//				eq(ConsultaSolicitudAclaracionesResponse.class), ArgumentMatchers.any()))
//						.thenThrow(new DataAccessException("Error Provocado") {});
//		callService(HttpMethod.POST, 404, Constants.ENDPOINT_GENERAL_ACLARACIONES + Constants.ENDPOINT_CONSULTA,
//				REQUEST_ACLARACIONES2);
//	}
	 ...
	 * @param status código de estatus HTTP que se espera de la ejecución de la
	 *               petición
